...
object Units {
    ...
    fun toUserUnitsSpd(v: Double?): Double? {
        ...
        return when (State.options.spdUnits) {
            C.SPD_MPH -> kt2mph(v)
            C.SPD_KPH -> kt2kph(v)
            else -> v
        }
    ...
    fun toUserUnitsDis(v: Double?): Double? {
        if (v == null) return null
        return when (State.options.distUnits) {
            C.DIS_SM -> nm2sm(v)
            C.DIS_KM -> nm2km(v)
            else -> v
        }
    ...
    fun toUserUnitsVol(v: Double?): Double? {
        if (v == null) return null
        return when (State.options.volUnits) {
